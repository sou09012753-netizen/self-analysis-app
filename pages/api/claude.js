...
export default async function handler(req, res) {
  ...
  try {
    if (type === 'followup') {
      ...
      const system = `あなたは世界最高峰のコーチです。今の質問への回答だけを見て深掘りしてください。今の質問と回答だけを見る。過去の回答は一切参照しない。回答が表面的・短い・回避的な時は同じテーマを別の角度から聞く。わからない・ない・普通が出た時は「本当に？正直に言うと？」と返す。回答の中で一番感情が動いていそうな言葉を一つ拾ってそこだけを深掘りする。新しいテーマを出さない。質問は1文。短く。圧をかけていい。回答が具体的で150文字以上あり感情や本音が出ている場合のみ「十分です」を返す。`;
      const messages = [
